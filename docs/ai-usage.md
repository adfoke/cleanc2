...
3. **退出码**（稳定承诺）：
   ...
   | 3 | 鉴权失败 | TCP 面需要 `-token` 或 `CLEANC2_TOKEN` |
   | 4 | 用法错误 | 重读 schema，别硬试 |
4. **零交互**：没有任何提示、确认等待、spinner。破坏性批量操作靠 `--yes` flag 显式声明。

## 连接

- 同机默认：什么都不用配，CLI 默认连 `./cleanc2.sock`（server 启动目录下的 UDS，免 token）。
- 远程/TCP：`-server http://host:8081 -token $T`，或 env `CLEANC2_SERVER` / `CLEANC2_TOKEN`。
- 全局 flag 可出现在 argv 任意位置（`cleanc2 agents list --pretty -server /x.sock` 合法）。
...
```bash
cleanc2 metrics overview          # {total_agents, online_agents, pending_tasks, ...}
cleanc2 agents list --online      # 在线清单
cleanc2 run --cmd "uptime" --agents id1 --wait   # 单机探活
```

### 批量执行（>1 目标必须 --yes）

```bash
cleanc2 run --cmd "df -h" --tag env=prod --yes --wait --exec-timeout 30
```
...
不 `--wait` 时立即返回 `{"tasks":[{task_id,agent_id,dispatched,queued_only}]}`；`queued_only=true` 表示 agent 离线、任务待补发。之后用 `cleanc2 tasks get <id>` 轮询。

### 文件

```bash
cleanc2 push --agent a1 --local ./app.bin --remote /opt/app.bin --wait
cleanc2 pull --agent a1 --remote /var/log/x.log --local ./x.log --wait
```
...
```bash
cleanc2 tasks list --limit 50
cleanc2 tasks cancel <task_id>   # 会终止 agent 上整棵进程树
```

### 分组

```bash
cleanc2 groups create --name web --agents a1,a2
cleanc2 groups add <group_id> a3 a4
cleanc2 groups remove <group_id> a2
```
